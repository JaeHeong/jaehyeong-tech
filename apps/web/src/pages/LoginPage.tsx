...
export default function LoginPage() {
  const { googleLogin, error, clearError, isAuthenticated, isLoading } = useAuth()
  ...
  const handleGoogleCallback = useCallback(
    ...
    [googleLogin, navigate, clearError, from]
  )
        ...
        <div className="bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-8">
          ...
          {!GOOGLE_CLIENT_ID ? (
            ...
            <div className="flex flex-col items-center gap-6">
              ...
              {/* Benefits */}
              <div className="w-full space-y-3 pt-4 border-t border-slate-200 dark:border-slate-700">
                <p className="text-sm font-medium text-slate-700 dark:text-slate-300 text-center mb-3">
                  로그인하면 이런 점이 좋아요
                </p>
                <div className="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <span className="material-symbols-outlined text-primary text-[18px] mt-0.5">check_circle</span>
                  <span>댓글 작성 시 이름과 프로필이 자동으로 표시됩니다</span>
                </div>
                <div className="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-400">
                  <span className="material-symbols-outlined text-primary text-[18px] mt-0.5">check_circle</span>
