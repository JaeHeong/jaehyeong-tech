      ...
      <div className="bg-card-light dark:bg-card-dark rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
        ...
        <div className="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-800">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold">버그 리포트 목록</h1>
            ...
            <Link
              to="/bug-report"
              className="px-3 sm:px-4 py-1.5 sm:py-2 bg-primary text-white font-medium rounded-lg hover:bg-primary/90 transition-all flex items-center gap-1.5 sm:gap-2 text-xs sm:text-sm w-fit"
            >
        ...
        {/* Filters */}
        <div className="p-3 sm:p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30">
          <div className="flex flex-wrap gap-2 sm:gap-3">
            <select
              value={statusFilter}
              onChange={(e) => { setStatusFilter(e.target.value as StatusFilter); setPage(1) }}
              className="flex-1 sm:flex-none rounded-lg bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs sm:text-sm px-2.5 sm:px-3 py-1.5"
            >
        ...
        <div className="divide-y divide-slate-100 dark:divide-slate-800">
          {isLoading ? (
            <div className="p-6 sm:p-8 text-center text-slate-500">
              <span className="material-symbols-outlined text-[20px] sm:text-[24px] animate-spin">progress_activity</span>
              <p className="mt-2 text-xs sm:text-sm">불러오는 중...</p>
            </div>
          ) : bugReports.length === 0 ? (
            <div className="p-6 sm:p-8 text-center text-slate-500">
              <span className="material-symbols-outlined text-[36px] sm:text-[48px] mb-2">inbox</span>
              <p className="text-xs sm:text-sm">등록된 버그 리포트가 없습니다.</p>
            </div>
          ) : (
            bugReports.map((report) => (
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2">
                  ...
                  <div className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap flex items-center gap-0.5 sm:gap-1">
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="p-3 sm:p-4 border-t border-slate-200 dark:border-slate-800 flex justify-center gap-1.5 sm:gap-2">
            <button
              onClick={() => setPage(Math.max(1, page - 1))}
              disabled={page === 1}
              className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
              이전
            </button>
            <span className="px-2.5 sm:px-3 py-1 sm:py-1.5 text-xs sm:text-sm text-slate-600 dark:text-slate-400">
              {page} / {totalPages}
            </span>
            <button
