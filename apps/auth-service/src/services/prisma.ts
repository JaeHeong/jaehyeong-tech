...
class TenantPrismaManager {
  ...
  getClient(tenantId: string): PrismaClient {
    ...
    console.info(`[Auth] Created Prisma client for tenant: ${tenantId}`);
    return client;
  }
  ...
  async disconnectAll(): Promise<void> {
    ...
    console.info('[Auth] All Prisma clients disconnected');
  }
}

export const tenantPrisma = new TenantPrismaManager();

// Legacy export for backward compatibility (used for tenant management operations)
